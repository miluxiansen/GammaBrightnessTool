...
public sealed class SettingsForm : Form
{
    ...
    private SettingsForm()
    {
        ...
        _navList.Items.Add(Localization.Get("SettingsBrightness"));
        _navList.Items.Add(Localization.Get("SettingsColorTemp"));
        _navList.Items.Add(Localization.Get("SolarAdjust"));
        ...
        _navList.FontChanged += (_, _) => KeepFontFixed(_navList, NavFixedFont);
    ...
    private void RefreshAllThemes()
    {
        ...
        foreach (var page in new[] { _generalPage, _brightnessPage, _hotkeysPage, _aboutPage, _colorTempPage, _solarPage })
        {
    ...
    private Panel BuildSettingRow(string labelText, Control rightControl)
    {
        ...
        inner.Layout += (_, _) =>
        {
            ...
            int textH = Math.Min(
                ...
                maxH);
            label.SetBounds(14, (inner.Height - textH) / 2, labelW, textH);
    ...
    private Panel BuildColorTempPage()
    {
        ...
        if (Program.Instance != null)
            Program.Instance.TemperatureChanged += (_, _) => refreshPresetSelection();
    ...
    private Panel BuildHotkeysPage()
    {
        ...
        // master is re-enabled, each toggle returns to its real saved state.
        bool masterOn = allHotKeysToggle.Checked;
        var hotKeyRows = new[]
        {
            (Row: incCapture, Getter: (Func<bool>)(() => Program.Instance?.GetIncreaseBrightnessHotKeyEnabled() ?? true)),
            (Row: decCapture, Getter: (Func<bool>)(() => Program.Instance?.GetDecreaseBrightnessHotKeyEnabled() ?? true)),
            (Row: powerOffCapture, Getter: (Func<bool>)(() => Program.Instance?.GetPowerOffHotKeyEnabled() ?? true)),
            (Row: incTempCapture, Getter: (Func<bool>)(() => Program.Instance?.GetIncreaseTemperatureHotKeyEnabled() ?? true)),
            (Row: decTempCapture, Getter: (Func<bool>)(() => Program.Instance?.GetDecreaseTemperatureHotKeyEnabled() ?? true))
        };
        foreach (var (row, getter) in hotKeyRows)
        {
            var toggle = FindHotKeyToggle(row);
            if (toggle == null) continue;
            // Start in the real state when master is on, forced off when off.
            toggle.Checked = masterOn && toggle.Checked;
            row.Enabled = masterOn && toggle.Checked;
            toggle.Enabled = masterOn;
            // Keep this sub-toggle consistent when the master flips. The
            // sub-toggle's own CheckedChanged fires on programmatic changes
            // too; when disabled it cannot be clicked, so no recursion.
            allHotKeysToggle.CheckedChanged += (_, _) =>
            {
                bool on = allHotKeysToggle.Checked;
                if (!on) toggle.Checked = false;
                else toggle.Checked = getter();
                toggle.Enabled = on;
                row.Enabled = on && toggle.Checked;
            };
        }
        ...
        bool enabled, Action<bool>? setEnabled)
    {
        ...
        // Row reflects the enable toggle: gray the whole row (label, capture
        // box, buttons) when this hotkey is switched off, or when the master
        // switch disables everything.
        row.Enabled = enabled && (Program.Instance?.GetAllHotKeysEnabled() ?? true);
    ...
    {
        if (c.Tag is string ts && (ts == "dynamic" || ts == "fontfix")) return;
        var target = c.Font;
        c.FontChanged += (_, _) =>
        {
            ...
            var f = c.Font;
            if (f.Size != target.Size || f.Unit != target.Unit) c.Font = target;
        };
        c.Tag = "fontfix";
