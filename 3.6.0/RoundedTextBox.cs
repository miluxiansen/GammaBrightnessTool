...
public class RoundedTextBox : TextBox
{
    ...
    // Native EDIT control margins (pixels). 6px keeps the text well clear of
    // the 6px rounded corners so the glyphs never sit on the arc itself.
    private const int EM_SETMARGINS = 0x00D3;
    private const int EC_LEFTMARGIN = 0x0001;
    private const int EC_RIGHTMARGIN = 0x0002;
    private const int TextLeftMargin = 6;
    private const int TextRightMargin = 6;
    private bool _marginsApplied;
    ...
    public virtual void ApplyTheme(Color background, Color foreground)
    {
        ...
        _marginsApplied = false; // handle re-create is rare, but keep flag honest
        Invalidate();
    }

    private void ApplyTextMargins()
    {
        if (_marginsApplied || !IsHandleCreated) return;
        SendMessage(Handle, EM_SETMARGINS,
            EC_LEFTMARGIN | EC_RIGHTMARGIN,
            (TextRightMargin << 16) | TextLeftMargin);
