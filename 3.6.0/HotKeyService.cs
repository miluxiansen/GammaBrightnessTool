...
public sealed class HotKeyService : IDisposable
{
    ...
    /// <summary>
    /// Win32 error code from the last RegisterHotKey attempt (0 on success).
    /// </summary>
    public int LastError { get; private set; }

    /// <summary>
    /// Registers a hotkey and reports success. Returns true on success. On
    /// failure the out parameter gets a short description and
    /// <see cref="LastError"/> holds the Win32 error code (e.g. 1409 =
    /// ERROR_HOTKEY_ALREADY_REGISTERED).
    /// </summary>
    public bool TryRegister(string hotkeyString, Action callback, out string error)
    {
        error = "";
        ...
        if (!RegisterHotKey(_windowHandle, id, modifiers, vk))
        {
            ...
            error = $"RegisterHotKey failed (error {LastError})";
            Debug.WriteLine($"[HotKeyService] Failed to register {hotkeyString}: {LastError}");
            return false;
        }
