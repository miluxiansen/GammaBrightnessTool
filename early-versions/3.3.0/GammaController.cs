...
public sealed class GammaController : IDisposable
{
    ...
    public void SetTemperature(float kelvin)
    {
        lock (_lock)
        {
            ...
            float snapped = Math.Clamp(kelvin, MIN_TEMPERATURE, MAX_TEMPERATURE);
    ...
    /// [MIN_TEMPERATURE, MAX_TEMPERATURE]. The wheel/hotkey callers pass
    ...
    public void AdjustTemperature(float deltaK)
    {
        lock (_lock)
        {
            ...
            next = Math.Clamp(next, MIN_TEMPERATURE, MAX_TEMPERATURE);
