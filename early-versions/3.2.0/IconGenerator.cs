...
public static class IconGenerator
{
    ...
    /// <summary>
    /// Creates a multi-resolution tray icon containing several sizes
    /// (16..256). Windows picks the frame closest to the actual display
    /// size, so the icon is rendered 1:1 instead of being bitmap-scaled.
    /// Bitmap scaling (StretchBlt-style) is what causes uneven stroke
    /// widths: at fractional scale factors horizontal/vertical lines land
    /// on pixel boundaries differently than the diagonal rays, so some
    /// strokes look thicker than others (and at 100% the thin vertical/
    /// horizontal strokes can be sampled away entirely). With a native
    /// frame at (or very near) the display size, no such scaling happens
    /// and every stroke renders at its designed pixel width.
    /// </summary>
    public static Icon CreateMultiSizeTrayIcon()
    {
        int[] sizes = { 16, 20, 24, 28, 32, 40, 48, 64, 96, 128, 256 };

        // Assemble: entries (fixed 16 bytes each) + image data blobs.
        // Build each size's PNG-encoded frame.
        ...
        foreach (int size in sizes)
        {
            ...
            using (var g = Graphics.FromImage(bitmap))
            {
                g.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
                g.Clear(Color.Transparent);
                // The tray glyph must contrast with the TASKBAR, which
                // follows the OS theme only (not the in-app theme choice).
