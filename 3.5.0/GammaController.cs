...
public sealed class GammaController : IDisposable
{
    ...
    public void SetTemperature(float kelvin)
    {
        ...
        {
            if (_paused) return;
            // 吸附到整数 K：存储值恒为整数，杜绝平滑动画终点残留
            // 6599.9998 之类的小数（显示/比较都干净）。
    ...
    /// hot-plug / resolution change / system resume: the old DCs may be
    /// stale or reset by the driver, so we re-enumerate and replay the
    /// current values. Safe to call repeatedly.
    /// </summary>
    public void RefreshDisplays()
    {
        lock (_lock)
        {
            ...
            // Replay current values so the screen reflects them again.
            ApplyGamma();
        }
    }
