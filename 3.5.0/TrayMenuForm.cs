...
internal sealed class TrayMenuForm : Form
{
    ...
    private void ShowSubmenuFor(int entryIndex)
    {
        ...
        if (entry.Sub == SubmenuKind.Disable)
        {
    ...
    private void BuildDisableSubMenu(TraySubMenu sub)
    {
        ...
        // 关闭：解除当前禁用；未禁用时置灰不可点。
        sub.AddChecked(Localization.Get("DisableOff"), false, () => OnDisableRequested?.Invoke(TimeSpan.Zero));
        if (!active) sub.DisableFirstItem();

        // 永久：激活且到期时间为 MaxValue 时勾选。
        sub.AddChecked(Localization.Get("DisablePermanent"), active && untilVal == DateTime.MaxValue, () => OnDisableRequested?.Invoke(null));
        ...
        sub.AddChecked(Localization.Get("Disable1Min"), checkedIdx == 0, () => OnDisableRequested?.Invoke(TimeSpan.FromMinutes(1)));
        sub.AddChecked(Localization.Get("Disable5Min"), checkedIdx == 1, () => OnDisableRequested?.Invoke(TimeSpan.FromMinutes(5)));
        sub.AddChecked(Localization.Get("Disable15Min"), checkedIdx == 2, () => OnDisableRequested?.Invoke(TimeSpan.FromMinutes(15)));
        sub.AddChecked(Localization.Get("Disable30Min"), checkedIdx == 3, () => OnDisableRequested?.Invoke(TimeSpan.FromMinutes(30)));
        sub.AddChecked(Localization.Get("Disable1Hour"), checkedIdx == 4, () => OnDisableRequested?.Invoke(TimeSpan.FromHours(1)));
        sub.AddChecked(Localization.Get("Disable3Hours"), checkedIdx == 5, () => OnDisableRequested?.Invoke(TimeSpan.FromHours(3)));
        sub.AddChecked(Localization.Get("Disable5Hours"), checkedIdx == 6, () => OnDisableRequested?.Invoke(TimeSpan.FromHours(5)));
        sub.AddChecked(Localization.Get("Disable12Hours"), checkedIdx == 7, () => OnDisableRequested?.Invoke(TimeSpan.FromHours(12)));
        sub.AddChecked(Localization.Get("Disable1Day"), checkedIdx == 8, () => OnDisableRequested?.Invoke(TimeSpan.FromDays(1)));
