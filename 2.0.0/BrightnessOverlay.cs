...
public sealed class BrightnessOverlay : Form
{
    ...
    /// <summary>
    /// Raised when the user clicks outside the popup (persistent mode) and the popup should close.
    /// </summary>
    public event EventHandler? OnDismissRequested;
    ...
    public BrightnessOverlay()
    {
        ...
        // Detect click-outside to dismiss persistent popup
        Deactivate += (s, e) =>
        {
            if (_isPersistent && Visible)
            {
                OnDismissRequested?.Invoke(this, EventArgs.Empty);
            }
        };
    }
    ...
    /// <summary>
    /// Closes the persistent popup (click-outside or explicit dismiss).
    /// </summary>
    public void Dismiss()
    {
        if (InvokeRequired)
