...
public sealed class BrightnessOverlay : Form
{
    ...
    /// <summary>数值文字的设计字号（pt）。与 <c>BrightnessPopup.ValueFontDesignPt</c> 同基准
    /// （两者本就互为镜像，3.6.0 注释 "mirror the wheel OSD"）：2148 版在 175% 下
    /// `int(7×1.75)` = 12pt @ 烘焙 168 ⇒ em 28px ⇒ 线性等效 12pt。</summary>
    ...
    public BrightnessOverlay()
    {
        ...
        // Percentage label
        // B'（2026-09-22）：px 值（设计 7pt 换算）；字体改 Pixel 单位。
        float fontSize = DpiFont.Px(dpiScale, ValueFontDesignPt);  // Even smaller font
        _label = new Label
        {
            ForeColor = ThemeManager.PopupText,
    ...
    {
    if (Frozen) return;   // 2026-09-16：单行 OSD 也要受冻结罩（此前只锁了多行路径）
    ...
    {
    if (Frozen) { _isDragging = false; return; }
    _isDragging = false;
    }
    ...
    /// <summary>
    /// 宿主注入的冻结锁（与弹窗 IsDisableActive 同语义 = 三个暂停源任一生效）。
    /// 2026-09-16：OSD 多行滑轨此前完全没有交互锁，暂停期间仍可拖动并改本地值。
    /// </summary>
    public Func<bool>? IsDisableActive { get; set; }
    private bool Frozen => IsDisableActive?.Invoke() == true;

    protected override void OnMouseDown(MouseEventArgs e)
    {
    base.OnMouseDown(e);
    if (Frozen) return;   // 冻结罩：暂停期间 OSD 行不响应拖动（2026-09-16）
    ...
    {
    base.OnMouseUp(e);
    if (Frozen) { _dragRowIndex = -1; Capture = false; return; }
    _dragRowIndex = -1;
        Capture = false;
    }
    ...
    /// <summary>P5：已注册自动化 ID 的行（型号短标识），避免"滚轮连调"反复重注册。</summary>
    ...
    /// <summary>
    /// P5 调试接口（2026-09-14）：把 OSD 滑轨暴露给自动化。与设置页同原则 ——
    /// **只驱动控件自身的值入口**（UpdateBrightness / 行的 SetBrightnessFromAutomation），
    /// 由既有事件处理器落业务，自动化与用户拖动走同一条代码。
    /// 主滑轨 <c>Osd_BriSlider</c>（0..100）+ 每屏行滑轨 <c>Osd_RowB_&lt;型号&gt;</c>。需在 UI 线程调用。
    /// </summary>
    public void RegisterAutomation()
    {
        AutomationBridge.Register(new AutomationAction("Osd_BriSlider", "slider",
        Get: () => _currentPercentage.ToString(CultureInfo.InvariantCulture),
        Set: v =>
        {
        if (Frozen) return;   // 2026-09-16：与真实鼠标路径同前置条件
    ...
    /// <summary>
    /// P5：按屏同步行滑轨的自动化注册（ID = <c>Osd_RowB_&lt;型号&gt;</c>）。
    /// OSD 行会被"滚轮连调"原位更新（ShowDisplays 快路径）反复触发，故仅屏集合变化时重注册。
    /// 需在 UI 线程调用。
    /// </summary>
    private void SyncRowAutomation()
    {
        var ids = new List<string>(_displayRows.Count);
        foreach (var r in _displayRows) ids.Add(AutomationBridge.ShortIdFromEdid(r.EdidId));
        ...
        AutomationBridge.ClearActionsByPrefix("Osd_RowB_");
        for (int i = 0; i < _displayRows.Count; i++)
        {
            string edid = _displayRows[i].EdidId;
            AutomationBridge.Register(new AutomationAction("Osd_RowB_" + ids[i], "slider",
                Get: () =>
                {
                    var row = FindRowByEdid(edid);
                    return row.HasValue
                        ? ((int)Math.Round(row.Value.Brightness * 100)).ToString(CultureInfo.InvariantCulture)
                        : "";
                },
                Set: v => { if (!Frozen) SetRowBrightnessFromAutomation(edid, v); }));
        }
    }

    private DisplayRow? FindRowByEdid(string edid)
    {
        foreach (var r in _displayRows)
        {
            if (string.Equals(r.EdidId, edid, StringComparison.OrdinalIgnoreCase)) return r;
        }
        return null;
    }

    /// <summary>P5：按 EDID 直接设定某行亮度（等价在该行滑轨上落定）；停用行不响应。</summary>
    private void SetRowBrightnessFromAutomation(string edid, string v)
    {
        if (!double.TryParse(v, NumberStyles.Float, CultureInfo.InvariantCulture, out double d) || !double.IsFinite(d)) return;
        float b = Math.Clamp((float)Math.Round(d) / 100f, 0f, 1f);
        for (int i = 0; i < _displayRows.Count; i++)
        {
            if (!string.Equals(_displayRows[i].EdidId, edid, StringComparison.OrdinalIgnoreCase)) continue;
            if (!_displayRows[i].Enabled) return;
            if (b == _displayRows[i].Brightness) return;
            _displayRows[i] = _displayRows[i] with { Brightness = b };
            if (i < _rowLabels.Count) _rowLabels[i].Text = $"{Math.Round(b * 100)}%";
            OnRowBrightnessChanged?.Invoke(edid, b);
            Invalidate();
            return;
        }
    }
    ...
    public void ShowDisplays(IReadOnlyList<DisplayRow> rows)
    {
        ...
        if (_multiRowMode && rows.Count > 0 && _rowLabels.Count == rows.Count)
        {
            ...
            SyncRowAutomation();   // P5：屏集合未变 → 内部直接返回，不重注册
            Invalidate();
        ...
        {
            // 空列表：隐藏多行并走单行逻辑
            SyncRowAutomation();   // P5：_displayRows 已空 → 清掉行滑轨自动化 ID
            ExitMultiRowMode();
            return;
        }
        ...
        for (int i = 0; i < rows.Count; i++)
        {
            var lbl = new Label
            {
                ForeColor = ThemeManager.PopupText,
                BackColor = Color.Transparent,
                // B'（2026-09-22）：px 值（设计 7pt 换算），Pixel 单位。
                Font = _cachedFont ?? new Font("Segoe UI", DpiFont.Px(dpiScale, ValueFontDesignPt), FontStyle.Bold, GraphicsUnit.Pixel),
        ...
        SyncRowAutomation();   // P5：按屏注册行滑轨自动化（Osd_RowB_<型号>）
        Invalidate();
        if (!Visible)
        {
            ShowNoActivate();
...
/// <summary>
/// 非激活显示 OSD（2026-09-16）。
/// 起因：实测日志证明直接 base.Show() 会让 OSD **抢前台** ——
///   `[hotkey] paused -> OSD only` 之后 20ms 就出现
///   `[fs] state=False fg=0x1305CC class='WindowsForms10.Window.20008.app...' pid=<本进程>`，
/// 于是 SystemEventMonitor 判定"脱离全屏"→ 解除全屏暂停（屏幕恢复设定值），
/// 1 秒后播放器拿回前台又判定全屏 → 再次暂停，观感即"恢复又暂停"。
/// 仅靠 WS_EX_NOACTIVATE / ShowWithoutActivation 不够，必须显式命令系统
/// 以 SW_SHOWNOACTIVATE 显示，并在被抢时把前台还给原窗口。
/// </summary>
private void ShowNoActivate()
{
    if (!Visible)
    {
        IntPtr prev = NativeMethods.GetForegroundWindow();
        // 2026-09-16 修正：首次也**不能**走 base.Show()！用户实测"进程内第一次唤 OSD"
        // 必现恢复又暂停、之后正常 —— 正是这条 base.Show() 分支在首次抢了前台。
        // 改为：只创建句柄（CreateParams 已含 WS_EX_NOACTIVATE，且窗口保持不可见）→ 不抢前台。
        bool createdBefore = IsHandleCreated;
        if (!createdBefore)
        {
            _ = Handle;   // 仅建句柄，不显示
        }
        ...
        // 兜底：万一仍被抢了前台，立刻还给原窗口（避免全屏应用失焦 → 误判脱离全屏）。
        if (prev != IntPtr.Zero && prev != Handle &&
            NativeMethods.GetForegroundWindow() == Handle)
        {
            NativeMethods.SetForegroundWindow(prev);
        }
        // 取证：确认 OSD 显示有没有抢走前台（stolen=True 仍在抢；restored 表示已兜回）。
        IntPtr afterFg = NativeMethods.GetForegroundWindow();
        OpLog.Log($"[osd] show createdBefore={createdBefore} prevFg=0x{prev.ToInt64():X} " +
                  $"afterFg=0x{afterFg.ToInt64():X} stolen={afterFg == Handle} selfPid={Environment.ProcessId}");
        // 异步复核（关键）：Windows 的激活是**滞后**的 —— 实测里同步读到 stolen=False，
        // 但 1ms 后 [fs] 就记录到前台变成了 OSD。同步检测抓不到，用 BeginInvoke 再看一次。
        BeginInvoke(new Action(() =>
        {
            IntPtr fg2 = NativeMethods.GetForegroundWindow();
            OpLog.Log(fg2 == Handle
                ? $"[osd] !! async STOLEN handle=0x{Handle.ToInt64():X}"
                : $"[osd] async ok fg=0x{fg2.ToInt64():X}");
        }));
    }
    else
    {
        // 已可见：只确保仍是 TopMost，绝不激活。
        NativeMethods.SetWindowPos(Handle, NativeMethods.HWND_TOPMOST, 0, 0, 0, 0,
    ...
    public void Show(float brightness, bool persistent)
    {
        ...
        if (DeviceDpi != _lastLayoutDpi)
        {
            ...
            // B'（2026-09-22）：px 值 + px 对 px 比较（原 `SizeInPoints` 比 pt，跨单位）。
            float fontSize = DpiFont.Px(dpiScale, ValueFontDesignPt);
            if (_cachedFont == null || _cachedFont.Size != fontSize)
            {
                _cachedFont?.Dispose();
                _cachedFont = new Font("Segoe UI", fontSize, FontStyle.Bold, GraphicsUnit.Pixel);
            }
    ...
    /// <summary>
    /// 鼠标点击 OSD 时**不激活**窗口（2026-09-16）。
    /// 起因：用户实测 —— 快捷键唤起 OSD 的问题已由 ShowNoActivate 解决，但**鼠标点到 OSD**
    /// 仍会把前台抢过来，于是全屏判定翻转 → 全屏暂停被解除 → 播放器拿回前台又暂停
    /// （观感"恢复又暂停"）。
    /// WM_MOUSEACTIVATE 返回 MA_NOACTIVATE：鼠标消息照常派发给窗口（滑轨仍按 Frozen 规则
    /// 决定能不能拖），但不夺取前台。用 MA_NOACTIVATE(3) 而非 MA_NOACTIVATEANDEAT(4)，
    /// 后者会把鼠标消息吃掉、控件收不到点击。
    /// </summary>
    protected override void WndProc(ref Message m)
    {
        ...
        {
            // 取证：记录一次"点击本想激活"的拦截（含当时前台，便于对照 [fs] 日志）。
            OpLog.Log($"[osd] WM_MOUSEACTIVATE -> MA_NOACTIVATE prevFg=0x{NativeMethods.GetForegroundWindow().ToInt64():X}");
