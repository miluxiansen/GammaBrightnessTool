...
public sealed class SettingSlider : Control
{
    ...
    /// <summary>
    /// 以「等价于用户拖动到该位置再松手」的方式设值：先设 <see cref="Value"/>（内部 clamp），
    /// 再依次触发 <see cref="ValueChanged"/> 与 <see cref="ValueCommitted"/>。
    ///
    /// 为什么需要它：<see cref="Value"/> 的 setter 只做 clamp + 重绘，**不发任何事件** ——
    /// 事件原本只由鼠标交互路径（UpdateFromX / OnMouseUp）触发。自动化或测试若直接赋
    /// Value，控件外挂的回调不会执行，表现为「值看着变了、设置其实没生效」。
    /// 值未变化时不发事件，与 ThemedComboBox.SelectedIndex 的语义保持一致。
    /// </summary>
    public void SetValueInteractive(float value)
    {
        float before = _value;
        Value = value;
        if (Math.Abs(_value - before) < 0.0001f) return;
