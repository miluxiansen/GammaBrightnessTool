...
{
    /// <summary>提示文字的设计字号（pt）。**基准 = 2148 版（B' 之前）在 175% 下的实际渲染**：
    /// 旧代码 `Math.Max(4,(int)(4*dpiScale))` 当 pt 用 ⇒ 175% 下 = `int(7.0)` = 7pt；
    /// 稳态烘焙 DPI 恒等于当前 DPI（见 BrightnessPopup 同名常数注释）⇒ em = 7 × 168/72 = 16.3px
    /// ⇒ B' 线性等效 = `16.3 ÷ (1.75 × 4/3)` = **7pt**。
    /// <br>⛔ 改动前先看 `_devtools/B_prime实施报告_20260922.md` §10。</summary>
    private const float TipFontDesignPt = 7f;
    ...
    public PowerTipForm()
    {
        ...
        _label = new Label
        {
            ...
            // B'（2026-09-22）：px 值 + GraphicsUnit.Pixel（原 `(int)(4 * dpiScale)` 当 pt 用，
            // 只在进程启动于 100% DPI 时才碰巧正确）。
            Font = DpiFont.Get(dpiScale, TipFontDesignPt),
