...
public sealed class BrightnessPopup : Form
{
    ...
    /// <summary>数值文字的设计字号（pt）。**基准 = 2148 版（B' 之前）在 175% 下的实际渲染**：
    /// 旧代码 `Math.Max(7,(int)(7*dpiScale))` 当 pt 用 ⇒ 175% 下 = `int(12.25)` = 12pt；
    /// 而**稳态下进程烘焙 DPI 恒等于当前 DPI**（每次改缩放都会自动重启，新进程即以新 DPI 为烘焙值，
    /// 见日志 `[dpi-probe] 旧(屏幕DC)=168`）⇒ 渲染 em = 12 × 168/72 = **28px**。
    /// B' 线性化复现 28px ⇒ `28 ÷ (1.75 × 4/3)` = **12pt**。
    /// <br>⛔ 改动前先看 `_devtools/B_prime实施报告_20260922.md` §10。</summary>
    private const float ValueFontDesignPt = 12f;
    ...
    public BrightnessPopup()
    {
        ...
        // B'（2026-09-22）：`fontSize` 语义由「pt × dpiScale」改为**像素**（设计 7pt 换算），
        // 字体同步改为 GraphicsUnit.Pixel。原写法 `(int)(7 * dpiScale)` 当 pt 用，
        // 实际渲染 px ≈ `7·dpiScale²·(96/72)·行高系数`（与烘焙 DPI 二次相关），
        // 只在进程启动于 100% DPI 时才碰巧正确。
        float fontSize = DpiFont.Px(dpiScale, ValueFontDesignPt);
    ...
    private void FitLabelFont()
    {
        ...
        // B'（2026-09-22）：`target` 现在是**像素**（`_labelBaseFontSize` 已由
        // `DpiFont.Px` 换算）。下限/级差仍是设计的 6pt / 0.5pt，经同一系数换算；
        // 探测字体也改用 Pixel 单位，与真实字体同源 —— 否则"量的"和"设的"不是同一个东西。
        // 本方法无 `_dpiScale` 字段，按控件所在屏现算。
        float fs = DeviceDpi / 96f;
        float floor = DpiFont.Px(fs, 6f);
        float step = Math.Max(0.05f, DpiFont.Px(fs, 0.5f));
        while (target > floor)
        {
            ...
            {
                Font = new Font(_label.Font.FontFamily.Name, target, _label.Font.Style, GraphicsUnit.Pixel),
                AutoSize = false,
                Text = _label.Text,
            };
        ...
        // B'：px 字体比 px（原为 `SizeInPoints` 比 pt，跨单位比较）
        if (Math.Abs(_label.Font.Size - target) > 0.01f)
        {
            ...
            oldFont?.Dispose();
            _label.Font = new Font(family, target, style, GraphicsUnit.Pixel);
        }
    }
    ...
    private void ApplyLayoutForCurrentDpi(int dpi, int widthPx)
    {
        ...
        // B'（2026-09-22）：px 值（设计 7pt 换算），字体改 Pixel 单位；比较也改为 px 对 px。
        float fontSize = DpiFont.Px(dpiScale, ValueFontDesignPt);
        _labelBaseFontSize = fontSize;
        if (_label.Font.Size != fontSize)
        {
            _label.Font?.Dispose();
            _label.Font = new Font("Segoe UI", fontSize, FontStyle.Bold, GraphicsUnit.Pixel);
        }
    ...
    /// <summary>P5：已注册自动化 ID 的行（型号短标识），避免行重建时重复注册。</summary>
    private readonly List<string> _autoRowIds = new();
    ...
    /// <summary>
    /// P5 调试接口（2026-09-14）：把弹窗的滑轨 / 模式开关暴露给自动化。
    /// 设计原则与设置页一致 —— **只驱动控件自身的值入口**（UpdateBrightness /
    /// UpdateTemperature / 行的 SetXxxFromAutomation），由既有事件处理器落业务，
    /// 因此自动化路径与用户拖动走同一条代码，不会出现"两套行为"分叉。
    /// 单屏主滑轨（Popup_BriSlider / Popup_TmpSlider）+ 每屏行滑轨（Popup_RowB_/RowT_，按 EDID 派生）。
    /// 需在 UI 线程调用。
    /// </summary>
    public void RegisterAutomation()
    {
        // 模式开关（亮度 ⇄ 色温）：ToggleSwitch，走通用 Wire（含 Get/Set/Click）。
        AutomationBridge.Wire((_modeSwitch, "Popup_ModeToggle"));

        // 主滑轨（单屏模式）：亮度 0..100 / 色温 K。Get 读控件自身状态，
        // Set 走与鼠标拖动同一个 UpdateXxx（内含 clamp、去重、事件派发）。
        AutomationBridge.Register(new AutomationAction("Popup_BriSlider", "slider",
        Get: () => _currentPercentage.ToString(CultureInfo.InvariantCulture),
        Set: v =>
        {
        if (IsDisableActive?.Invoke() == true) return;   // 2026-09-16：与真实鼠标路径同前置条件
        if (double.TryParse(v, NumberStyles.Float, CultureInfo.InvariantCulture, out double d)
        && double.IsFinite(d))
                {
                    UpdateBrightness(Math.Clamp((int)Math.Round(d), 0, 100));
                }
            }));
        AutomationBridge.Register(new AutomationAction("Popup_TmpSlider", "slider",
        Get: () => ((int)Math.Round(_currentTemperatureK)).ToString(CultureInfo.InvariantCulture),
        Set: v =>
        {
        if (IsDisableActive?.Invoke() == true) return;   // 2026-09-16：与真实鼠标路径同前置条件
        if (double.TryParse(v, NumberStyles.Float, CultureInfo.InvariantCulture, out double d)
        && double.IsFinite(d))
                {
                    UpdateTemperature((float)d);
                }
            }));
    }

    /// <summary>
    /// P5：按屏同步行滑轨的自动化注册（ID = <c>Popup_RowB_&lt;型号&gt;</c> / <c>Popup_RowT_&lt;型号&gt;</c>）。
    /// 仅在屏集合变化时重注册；离开独立模式（传空）时清掉，避免陈旧 ID 指向已 Dispose 的行。
    /// 需在 UI 线程调用。
    /// </summary>
    private void SyncRowAutomation(IReadOnlyList<DisplayRowData> rows)
    {
        var ids = new List<string>(rows.Count);
        foreach (var r in rows) ids.Add(AutomationBridge.ShortIdFromEdid(r.EdidId));

        if (ids.Count == _autoRowIds.Count)
        {
            bool same = true;
            for (int i = 0; i < ids.Count; i++)
            {
                if (!string.Equals(ids[i], _autoRowIds[i], StringComparison.Ordinal)) { same = false; break; }
            }
            if (same) return;   // 屏集合未变 → 已注册的仍然有效
        }

        _autoRowIds.Clear();
        _autoRowIds.AddRange(ids);
        AutomationBridge.ClearActionsByPrefix("Popup_RowB_");
        AutomationBridge.ClearActionsByPrefix("Popup_RowT_");
        for (int i = 0; i < rows.Count; i++)
        {
            string edid = rows[i].EdidId;
            string sid = ids[i];
            AutomationBridge.Register(new AutomationAction("Popup_RowB_" + sid, "slider",
                Get: () => RowValueText(edid, brightness: true),
                Set: v => SetRowValueFromAutomation(edid, v, brightness: true)));
            AutomationBridge.Register(new AutomationAction("Popup_RowT_" + sid, "slider",
                Get: () => RowValueText(edid, brightness: false),
                Set: v => SetRowValueFromAutomation(edid, v, brightness: false)));
        }
    }

    /// <summary>P5：按 EDID 找当前行（行对象随重建被替换，故按 EDID 查而非闭包捕获旧行）。</summary>
    private PopupDisplayRow? FindRowByEdid(string edid)
    {
        foreach (var row in _rowControls)
        {
            if (string.Equals(row.EdidId, edid, StringComparison.OrdinalIgnoreCase)) return row;
        }
        return null;
    }

    private string RowValueText(string edid, bool brightness)
    {
        var row = FindRowByEdid(edid);
        if (row == null) return "";
        return brightness
            ? ((int)Math.Round(row.Brightness * 100)).ToString(CultureInfo.InvariantCulture)
            : ((int)Math.Round(row.Temperature)).ToString(CultureInfo.InvariantCulture);
    }

    private void SetRowValueFromAutomation(string edid, string v, bool brightness)
    {
        if (!double.TryParse(v, NumberStyles.Float, CultureInfo.InvariantCulture, out double d) || !double.IsFinite(d)) return;
        var row = FindRowByEdid(edid);
        if (row == null) return;
        if (brightness) row.SetBrightnessFromAutomation((float)(d / 100.0));
        else row.SetTemperatureFromAutomation((float)d);
    }
    ...
    private void ApplyDisplayRows()
    {
        ...
        if (!_perMonitorEnabled)
        {
            // P5：离开独立模式 → 行滑轨不复存在，清掉其自动化 ID。
            SyncRowAutomation(Array.Empty<DisplayRowData>());
        ...
        for (int i = 0; i < _displayRows.Count; i++)
        {
            ...
            row.IsLocked = IsDisableActive;   // 2026-09-16：把弹窗的冻结锁下发给每一行
        ...
        // P5：按屏注册行滑轨自动化（屏集合未变则内部直接返回）。
        SyncRowAutomation(_displayRows);
    }
...
int labelHeight = (int)(20 * dpiScale);
    ...
    private sealed class PopupDisplayRow : Control
    {
        ...
        /// <summary>
        /// 宿主注入的冻结锁（= 弹窗的 IsDisableActive：三个暂停源任一生效即为 true）。
        /// 2026-09-16：多屏行此前**完全没有**交互锁 —— 暂停期间拖动仍会改写行本地字段并重绘，
        /// 于是"数值变了"，而 MainController 的写入守卫把值吞掉 → 关窗再开数值"恢复"。
        /// </summary>
        public Func<bool>? IsLocked { get; set; }
        private bool Frozen => IsLocked?.Invoke() == true;
        ...
        /// <summary>
        /// P5 调试接口：按"值"直接设定行亮度（等价滚轮一格：改字段 + 发事件 + 重绘）。
        /// 停用行冻结，不响应（与 OnMouseWheel 同守卫）。
        /// </summary>
        public void SetBrightnessFromAutomation(float brightness)
        {
        if (!_enabled || Frozen) return;
            float b = Math.Clamp((float)Math.Round(brightness * 100) / 100f, 0f, 1f);
            if (Math.Abs(b - _brightness) < 0.0001f) return;
            _brightness = b;
        ...
        /// <summary>
        /// P5 调试接口：按"值"直接设定行色温（取整、clamp 到该行范围）。
        /// 仅色温模式有意义（与 OnMouseWheel 的 _temperatureMode 守卫一致）。
        /// </summary>
        public void SetTemperatureFromAutomation(float kelvin)
        {
            if (!_enabled || Frozen || !_temperatureMode) return;
            float t = (float)Math.Round(Math.Clamp(kelvin, _minTemp, _maxTemp));
            if (Math.Abs(t - _temperature) < 0.5f) return;
            _temperature = t;
        ...
        {
        base.OnMouseDown(e);
        if (Frozen) return;   // 冻结罩：暂停期间整行不响应鼠标（2026-09-16）
        if (e.Button == MouseButtons.Left)
            {
                _dragging = true;
                _tempDwellTimer?.Dispose();
        ...
        {
        base.OnMouseUp(e);
        if (Frozen) { _dragging = false; return; }
        _dragging = false;
            long now = Environment.TickCount64;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            // B'（2026-09-22）：px 值（设计 7pt 换算），下方字体同用 Pixel 单位。
            float fontSize = DpiFont.Px(_dpiScale, ValueFontDesignPt);
            ...
            using (var nameFont = new Font("Segoe UI", fontSize, FontStyle.Regular, GraphicsUnit.Pixel))
            {
            ...
            using (var valueFont = new Font("Segoe UI", fontSize, FontStyle.Bold, GraphicsUnit.Pixel))
            {
