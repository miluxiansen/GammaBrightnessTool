...
public sealed class ToggleSwitch : Control
{
    ...
    private int _animSteps;      // 本次滑块动画已走步数（取证：分「走完」与「被 Dispose 打断」）
    ...
    private void StartAnimation()
    {
        ...
        _animTimer.Tick += (_, _) =>
        {
            ...
            if (Math.Abs(target - _knobPos) < 0.02f)
            {
                ...
                _animTimer.Dispose();
                _animTimer = null;
                // 取证（正式版 OpLog 为空实现，零开销）：动画完整走完
                OpLog.Log($"[toggle/anim] 滑块动画走完 {_animSteps} 步");
            }
            Invalidate();
        };
        _animTimer.Start();
    ...
    protected override void Dispose(bool disposing)
    {
        if (disposing)
        {
            ...
            if (_animTimer != null)
            {
                // 取证：动画被销毁打断 ⇒ 滑块会「瞬移」到位（用户观感 = 没有动画）
                OpLog.Log($"[toggle/anim] ✗ 动画被 Dispose 打断，只走了 {_animSteps} 步");
            }
            _animTimer?.Stop();
            _animTimer?.Dispose();
            _animTimer = null;
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        // settings switches follow the main UI theme (IsDark). Disabled
        // switches keep the knob gray so an "off due to master switch"
        // state reads as inactive (the knob color follows the switch state,
        // NOT the Enabled property).
        ...
        if (!Enabled) trackColor = dark ? Color.FromArgb(70, 70, 72) : Color.FromArgb(210, 210, 212);
        ...
        using (var knobBrush = new SolidBrush(Enabled ? Color.White : Color.FromArgb(215, 215, 218)))
        {
