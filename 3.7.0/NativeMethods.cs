...
public static class NativeMethods
{
    ...
    /// <summary>
    /// 注册一个系统级广播消息，返回其消息 ID（同一字符串在全系统返回同一值）。
    /// 用于监听 explorer 广播的 "TaskbarCreated" —— 任务栏（重新）建立时发出，
    /// 是「通知图标已全部丢失、必须重新 Shell_NotifyIcon(NIM_ADD)」的官方信号。
    /// 图标活在 explorer 进程内存里，explorer 重启（崩溃自恢复 / 用户手动重启 /
    /// 大版本升级期间 shell 多次重启）后不会自动重建，只能靠本消息重新注册。
    /// </summary>
    ...
    #region Console（CLI 诊断输出）

    /// <summary>把本进程挂到父进程的控制台。</summary>
    [DllImport("kernel32.dll", SetLastError = true)]
    public static extern bool AttachConsole(uint dwProcessId);

    /// <summary>
    /// AttachConsole 的特殊值：挂到"父进程的控制台"。
    /// 本程序 OutputType=WinExe，默认没有控制台，`--auto` 的 JSON 结果直接
    /// Console.Write 是看不到的 —— 必须先挂上父控制台，脚本才能捕获 stdout。
    /// </summary>
    ...
    /// <summary>
    /// Enumerates display adapters (first call) or monitors attached to a
    /// specific adapter (subsequent calls with lpDevice set).
    /// Level 2 (monitor) DeviceID carries the EDID instance ID
    /// (MONITOR\VendorModel\{GUID}\Instance), used as the stable key
    /// for per-monitor state in 3.6.0.
    /// </summary>
    ...
    #region DisplayConfig (monitor friendly names)

    /// <summary>LUID (local unique identifier) used for adapter identity.</summary>
    [StructLayout(LayoutKind.Sequential)]
    public struct LUID
    {
        public uint LowPart;
    ...
    public struct DISPLAYCONFIG_PATH_TARGET_INFO
    {
        ...
        public int rotation;
        public int scaling;
        public uint refreshRateNumerator;   // DISPLAYCONFIG_RATIONAL = 2x uint (4-byte aligned, NOT long!)
    ...
    public const int QDC_ONLY_ACTIVE_PATHS = 1;
    ...
    [DllImport("user32.dll")]
    public static extern int QueryDisplayConfig(int flags, ref uint numPathArrayElements, [Out] DISPLAYCONFIG_PATH_INFO[] pathInfoArray, ref uint numModeInfoArrayElements, IntPtr modeInfoArray, IntPtr currentTopologyId);
    ...
    /// <summary>ShowWindow 命令：显示但不激活 —— 2026-09-16 OSD 防抢前台用。</summary>
    public const int SW_SHOWNOACTIVATE = 4;

    /// <summary>SetWindowPos 标志：显示窗口（配合 SWP_NOACTIVATE 即"显示但不激活"）。</summary>
    ...
    // 2026-09-16：OSD 非激活显示后的兜底 —— 若仍被抢了前台就还给原窗口。
    [DllImport("user32.dll")]
