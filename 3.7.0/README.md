# Gamma Brightness Tool - 3.7.0

> **English** | [中文](#中文版--gamma-brightness-tool-370)
...
Download `GammaBrightnessTool-Portable-v3.7.0.zip`:
...
├── SettingsForm.cs               # Settings window (8 pages, instant theme refresh)
...
├── AppWhitelistService.cs        # App whitelist: pause while a listed app is visible
├── TrayVisibilityService.cs      # Win11 overflow-menu icon visibility control
├── AutomationBridge.cs           # Automation bridge (control ids + named-pipe actions)
...
# 中文版 | Gamma Brightness Tool 3.7.0

> [English](#gamma-brightness-tool---370) | **中文**
...
### 方式二：绿色版（推荐）
下载 `GammaBrightnessTool-Portable-v3.7.0.zip`：
...
├── SettingsForm.cs               # 设置窗口（8 页导航、主题即时刷新）
├── HotKeyService.cs              # 全局热键注册（WM_HOTKEY）
├── IconGenerator.cs              # 多尺寸托盘图标组装
├── TrayMenuForm.cs               # 自绘托盘菜单（亮度挡位 / 语言 / 禁用）
├── AppWhitelistService.cs        # 应用白名单（列出的程序可见时自动暂停）
├── TrayVisibilityService.cs      # Win11 溢出菜单图标可见性控制
├── AutomationBridge.cs           # 自动化桥（控件 ID + 命名管道动作注册表）
