...
public sealed class HotKeyCaptureBox : RoundedTextBox
{
    ...
    // + `FontChanged` 处理器"把被 WinForms 缩放的字体拉回此实例"。
    // 该静态字段在 B' 下不可用：① 静态初始化拿不到 dpiScale；② 需要"拉回"的前提
    // 正是 pt 字体被隐式缩放。现改为按 `DeviceDpi` **现算 px 字体**（见 RefreshDpiFont），
    // `GraphicsUnit.Pixel` 不会被缩放 ⇒ 补丁与静态字段一并删除。
    ...
    public HotKeyCaptureBox()
    {
        ...
        Cursor = Cursors.Hand;
        // B'：px 字体（设计 10pt）。句柄创建前 `DeviceDpi` 可能是进程启动时的陈旧值
        // ⇒ OnHandleCreated 会重算一次；此后 DPI 变化由 OnDpiChangedAfterParent 兜住。
        RefreshDpiFont();
    }

    /// <summary>
    /// B'（2026-09-22）：按控件所在屏 DPI 重建 px 字体（设计 10pt，与设置页其他文字一致）。
    /// `GraphicsUnit.Pixel` ⇒ WinForms 的 PerMonitorV2 隐式 pt 缩放对本控件字体完全失效，
    /// 因此不再需要任何 FontChanged 补丁。
    /// </summary>
    ...
    protected override void OnHandleCreated(EventArgs e)
    {
        ...
        // 句柄建好后 `DeviceDpi` 才可靠（ctor 里可能是陈旧值）⇒ 按真实值重算。
        RefreshDpiFont();
    }
