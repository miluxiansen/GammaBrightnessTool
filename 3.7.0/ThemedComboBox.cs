...
public sealed class ThemedComboBox : Control
{
    ...
    // B'（2026-09-22）：原为 `static readonly Font FixedFont = new Font("Segoe UI", 10F)`
    // + `FontChanged` 处理器"把被 WinForms 按 newDpi/oldDpi 缩放的字体拉回此实例"。
    // 该静态字段在 B' 下不可用：① 静态初始化拿不到 dpiScale；② 需要"拉回"的前提
    // 正是 pt 字体被隐式缩放（主控件绘制用 `Font`、下拉弹窗绘制用 `_owner.Font`，
    // 两处都会被改）。现改为按 `DeviceDpi` **现算 px 字体**（见 RefreshDpiFont），
    // `GraphicsUnit.Pixel` 不会被隐式缩放 ⇒ 补丁与静态字段一并删除。
    ...
    public ThemedComboBox()
    {
        ...
        // B'：px 字体（设计 10pt，与设置页选项文字一致）。句柄创建前 `DeviceDpi`
        // 可能是陈旧值 ⇒ OnHandleCreated 重算一次；此后 DPI 变化由
        // OnDpiChangedAfterParent 兜住。
        RefreshDpiFont();
    }

    /// <summary>
    /// B'（2026-09-22）：按控件所在屏 DPI 重建 px 字体（设计 10pt）。
    /// `GraphicsUnit.Pixel` ⇒ 主控件绘制与下拉弹窗绘制都拿到同一个不被缩放的字体，
    /// 因此不再需要 FontChanged 补丁。
    /// </summary>
    private void RefreshDpiFont()
        => Font = DpiFont.Get(DeviceDpi / 96f, 10f);

    protected override void OnHandleCreated(EventArgs e)
    {
        base.OnHandleCreated(e);
        // 句柄建好后 `DeviceDpi` 才可靠（ctor 里可能是进程启动时的陈旧值）。
        RefreshDpiFont();
    }
...
internal sealed class DropdownListPopup : Form, IMessageFilter
{
    ...
    /// <summary>打开弹窗前宿主的 TopMost 值；关闭时恢复（见 ShowPopup 里的说明）。</summary>
    private bool _ownerWasTopMost;
    ...
    public void ShowAt(ThemedComboBox owner)
    {
        ...
        if (owner.TopLevelControl is Form topForm)
        {
            ...
            // 置顶（图钉）时：**临时取消宿主的 TopMost**，而不是把弹窗设成 TopMost。
            //   · 弹窗按设计不是 TopMost（见 :299），TopMost 的宿主会盖住它 → 下拉点不开；
            //   · 但反过来把弹窗设成 TopMost 也不行：它的创建会触发宿主 Deactivate，
            //     200ms 后"是否点在弹窗内"的检查判定为用户已离开 → **展开即缩回**（用户实测）。
            // 取消宿主 TopMost 后，owner 关系让弹窗自然浮在宿主之上；关闭时在 ClosePopup 恢复。
            _ownerWasTopMost = topForm.TopMost;
            if (_ownerWasTopMost) topForm.TopMost = false;
        }
        Show();
    ...
    public void ClosePopup()
    {
        ...
        if (_ownerForm != null)
        {
            ...
            // 恢复打开弹窗前宿主的置顶状态（见 ShowPopup：为避免"下拉点不开"临时取消过）。
            if (_ownerWasTopMost)
            {
                _ownerWasTopMost = false;
                try { _ownerForm.TopMost = true; } catch { /* best-effort */ }
            }
            _ownerForm = null;
        }
        if (!IsDisposed) Hide();
