...
/// (通用设置 / 热键绑定 / 版本信息) and a right content area.
...
public sealed class SettingsForm : Form
{
    ...
    private SettingsForm()
    {
        ...
        _hotkeysPage = BuildPlaceholderPage(Localization.Get("SettingsPlaceholder"));
    ...
    private Panel BuildGeneralPage()
    {
        ...
        langCombo.ApplyTheme(BgInner, TextMain);
        ...
        themeCombo.ApplyTheme(BgInner, TextMain);
        ...
        popupThemeCombo.ApplyTheme(BgInner, TextMain);
        // Index 0 = follow system; 1 = dark; 2 = light.
        ...
        stepCombo.ApplyTheme(BgInner, TextMain);
        // Index 0..3 map to 5/10/15/20%.
        stepCombo.Items.Add(Localization.Get("StepSize5"));
        stepCombo.Items.Add(Localization.Get("StepSize10"));
        stepCombo.Items.Add(Localization.Get("StepSize15"));
        stepCombo.Items.Add(Localization.Get("StepSize20"));
        float savedStep = Program.Instance?.GetStepSize() ?? GammaController.DEFAULT_STEP;
        stepCombo.SelectedIndex = savedStep <= 0.075f ? 0 : savedStep <= 0.125f ? 1 : savedStep <= 0.175f ? 2 : 3;
        stepCombo.SelectedIndexChanged += (_, _) =>
        {
            float[] steps = { 0.05f, 0.10f, 0.15f, 0.20f };
            Program.Instance?.SetStepSize(steps[stepCombo.SelectedIndex]);
        };
        ...
        page.Controls.Add(overlayRow);

        invertRow.Dock = DockStyle.Top;
        page.Controls.Add(invertRow);

        stepRow.Dock = DockStyle.Top;
        page.Controls.Add(stepRow);

        popupThemeRow.Dock = DockStyle.Top;
        page.Controls.Add(popupThemeRow);

        themeRow.Dock = DockStyle.Top;
        page.Controls.Add(themeRow);

        langRow.Dock = DockStyle.Top;
        page.Controls.Add(langRow);

        startupRow.Dock = DockStyle.Top;
        page.Controls.Add(startupRow);
        var title = new Label
        {
    ...
    private Panel BuildAboutPage()
    {
        ...
        checkUpdateBtn.FlatAppearance.BorderColor = Border;
